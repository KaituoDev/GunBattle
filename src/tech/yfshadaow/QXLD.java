...
public class QXLD extends JavaPlugin implements Listener {
    ...
    public void preventSwapHand(PlayerSwapHandItemsEvent pshie) {
        if (pshie.getPlayer().getScoreboardTags().contains("qxld")) {
    ...
    public void resetAmmoAmount(PlayerRespawnEvent pre) {
        ...
        Bukkit.getScheduler().runTaskLater(this,() -> {
            if (!p.getScoreboardTags().contains("qxld")) {
                return;
            }
    ...
    public void resistKnockback(EntityDamageEvent ede) {
        if (!ede.getEntity().getScoreboardTags().contains("qxld")) {
            return;
        }
    ...
    public void reduceAttackRange(EntityDamageByEntityEvent edbee) {
        ...
        if (!edbee.getDamager().getScoreboardTags().contains("qxld")) {
            return;
        }
    ...
    public void reloadAmmo(PlayerDropItemEvent pdie) {
        if (!pdie.getPlayer().getScoreboardTags().contains("qxld")) {
            return;
        }
    ...
    public void onProjectileHit(ProjectileHitEvent phe) {
        if (!((phe.getEntity().getShooter()) instanceof Entity)) { return; }//不是实体
        Entity shooter = (Entity)phe.getEntity().getShooter();
        if (!shooter.getScoreboardTags().contains("qxld")) { return; }//不在里
    ...
    public void cancelItemMovement (InventoryClickEvent ice) {
        ...
        if (ice.getWhoClicked().getScoreboardTags().contains("qxld")) {
    ...
    public void giveItems(PlayerDeathEvent pde) {
        ...
        if (scoreboard.getTeam("qxldR").hasPlayer(p)) {
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(),"execute as " + p.getName() + " run function qxld:red");
        } else if (scoreboard.getTeam("qxldB").hasPlayer(p)) {
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(),"execute as " + p.getName() + " run function qxld:blue");
        }
    }
    @EventHandler
    public void onPlayerDeath(PlayerDeathEvent pde) {
        ...
        if (!player.getScoreboardTags().contains("qxld")) {
            return;
        }
        if (killer == null) {
            return;
        ...
        if (killer.getType().equals(EntityType.PLAYER)) {
            ...
            if (killer.getScoreboardTags().contains("qxld")) {
                if (scoreboard.getTeam("qxldR").hasPlayer((Player)killer)) {
                    Score score = qxld.getObjective("qxld").getScore("§c" + killer.getName());
                    score.setScore(score.getScore() + 1);//加分
                } else if (scoreboard.getTeam("qxldB").hasPlayer((Player)killer)) {
                    Score score = qxld.getObjective("qxld").getScore("§9" + killer.getName());
                    score.setScore(score.getScore() + 1);//加分
                }
            }
        }
    ...
    public void clearKills(PlayerInteractEvent pie) {
        ...
        if (x == 1 && y == 60 && z == 1990) {
            qxld.getObjective("qxld").unregister();
            qxld.registerNewObjective("qxld","dummy","枪械乱斗击败榜");
            qxld.getObjective("qxld").setDisplaySlot(DisplaySlot.SIDEBAR);
        }
    }
    ...
    public void preventRegen(EntityRegainHealthEvent erhe) {
        ...
        if (!erhe.getEntity().getScoreboardTags().contains("qxld")) {
            return;
        }
    ...
    public void onEnable() {
        ...
        qxld = Bukkit.getScoreboardManager().getMainScoreboard();
